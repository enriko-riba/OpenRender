...
internal sealed class TerrainGenerationProfiler
{
    ...
    public void LogStatistics()
    {
        ...
        Trace.TraceInformation($"=== Terrain Generation Performance ({_totalChunks} chunks) ===");
        Trace.TraceInformation($"Climate:  avg={GetAverageMs(Step.ClimateSampling):F2}ms, " +
                  $"max={GetMaxMs(Step.ClimateSampling):F2}ms, " +
                  $"min={GetMinMs(Step.ClimateSampling):F2}ms");
        Trace.TraceInformation($"Height:   avg={GetAverageMs(Step.HeightCalculation):F2}ms, " +
                  $"max={GetMaxMs(Step.HeightCalculation):F2}ms, " +
                  $"min={GetMinMs(Step.HeightCalculation):F2}ms");
        Trace.TraceInformation($"3D Noise: avg={GetAverageMs(Step.Noise3DSampling):F2}ms, " +
                  $"max={GetMaxMs(Step.Noise3DSampling):F2}ms, " +
                  $"min={GetMinMs(Step.Noise3DSampling):F2}ms");
        Trace.TraceInformation($"Biome:    avg={GetAverageMs(Step.BiomeSelection):F2}ms, " +
                  $"max={GetMaxMs(Step.BiomeSelection):F2}ms, " +
                  $"min={GetMinMs(Step.BiomeSelection):F2}ms");
        Trace.TraceInformation($"Blocks:   avg={GetAverageMs(Step.BlockGeneration):F2}ms, " +
                  $"max={GetMaxMs(Step.BlockGeneration):F2}ms, " +
                  $"min={GetMinMs(Step.BlockGeneration):F2}ms");
        Trace.TraceInformation($"TOTAL:    avg={GetAverageMs(Step.Total):F2}ms, " +
        ...
        {
            Trace.TraceWarning($"Total chunk generation exceeds 10ms budget! " +
                       $"(avg={GetAverageMs(Step.Total):F2}ms)");
        }
        if (GetAverageMs(Step.ClimateSampling) > 2.0)
        {
            Trace.TraceWarning($"Climate sampling exceeds 2ms budget! " +
