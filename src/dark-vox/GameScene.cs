...
using DarkVox.Client.Mobs;
using DarkVox.Client.Rendering;
using DarkVox.Client.Terrain;
...
internal class GameScene : Scene
{
    ...
    public override void Load()
    {
        ...
        droppedItemRenderer ??= new DroppedItemRenderer(this);
    ...
    private void RenderUI()
    {
        ...
        // Show server movement state via velocity (client RequestedMovement isn't updated since Simulate() isn't called)
        var vel = player.Velocity;
        var isMovingOnServer = (vel.X * vel.X + vel.Z * vel.Z) > 0.01f; // Check horizontal velocity
        var movingIndicator = isMovingOnServer ? "MOV" : "---";
        WriteLine($"  HP: {a.Health}/{a.MaxHealth} | Food: {a.Food}/{a.MaxFood} | Sat: {a.Saturation:F1} | Exh: {a.Exhaustion:F2} {movingIndicator}", textColor);
        ...
        WriteLine("  Ctrl - Sprint (run)", textColor);
        WriteLine("  Shift - Crouch/Sneak", textColor);
        WriteLine("  Space - Jump", textColor);
