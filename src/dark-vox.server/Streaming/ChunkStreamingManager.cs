...
using DarkVox.World; // Added to access ChunkVoxelDataCache, ChunkBiomeData, and ChunkProcessingMetrics

namespace DarkVox.Server.Streaming;
...
public sealed class ChunkStreamingManager : IDisposable, IBlockEditService
{
    ...
    /// <summary>
    /// Gets the on-disk folder that contains all save data for the currently loaded world.
    /// </summary>
    public string GetWorldSaveDirectory()
    {
        var folderName = $"{terrainConfig.WorldName}_{generationSeed}";
        return Path.Combine(ResolveDataRoot(), SaveRootFolderName, folderName);
    }
    ...
    private static bool TryLoadChunkStateFromPath(string path, int chunkIdx, out ChunkData? data, out ChunkBiomeData? biomeData)
    {
        ...
        try
        {
            ...
            {
                System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Invalid chunk state header; expectedMagic='{ChunkStateMagic}' actualMagic='{magic}' chunkIdx={chunkIdx} fileSizeBytes={fileSizeBytes} path='{path}'");
            ...
            if (data == null)
            {
                System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Failed to deserialize chunk state (null) chunkIdx={chunkIdx} version={version} fileSizeBytes={fileSizeBytes} path='{path}'");
                TryQuarantineCorruptChunkStateFile(path);
                return false;
            }
            ...
            {
                System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Invalid/corrupt chunk state; reason='{invalidReason}' chunkIdx={chunkIdx} serializedChunkIdx={data.ChunkIndex} version={version} fileSizeBytes={fileSizeBytes} path='{path}'");
                TryQuarantineCorruptChunkStateFile(path);
                data = null;
                biomeData = null;
                return false;
        ...
        {
            System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Exception while loading chunk state; chunkIdx={chunkIdx} magic='{magic ?? "<unread>"}' version='{(version.HasValue ? version.Value.ToString() : "<unread>")}' fileSizeBytes={fileSizeBytes} exType='{ex.GetType().Name}' exMessage='{ex.Message}' path='{path}'");
            TryQuarantineCorruptChunkStateFile(path);
            return false;
        }
    }
    ...
    private void SaveDirtyEdits(bool force)
    {
        ...
        if (saved > 0)
        {
            System.Diagnostics.Trace.TraceInformation($"Server ChunkStreamingManager: Saved edits for {saved} chunks");
        }
    }
    ...
    private bool TrySaveChunkEdits(int chunkIdx)
    {
        ...
        {
            System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Failed to save edits for chunk {chunkIdx}: {ex.Message}");
            return false;
        }
    }
    ...
    private void LoadEdits()
    {
        ...
        if (loadedCount > 0)
        {
            System.Diagnostics.Trace.TraceInformation($"Server ChunkStreamingManager: Loaded edits for {loadedCount} chunks from {dirPath}");
        }
    }
    ...
    private static bool TrySaveChunkStateToPath(string path, ChunkData data, ChunkBiomeData? biomeData)
    {
        ...
        {
            System.Diagnostics.Trace.TraceWarning($"Server ChunkStreamingManager: Failed to save chunk state; chunkIdx={data.ChunkIndex} exType='{ex.GetType().Name}' exMessage='{ex.Message}' path='{path}'");
            return false;
        }
    }
