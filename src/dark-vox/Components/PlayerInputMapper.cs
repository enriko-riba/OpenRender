...
public static class PlayerInputMapper
{
    public static PlayerInputState BuildState(KeyboardState keyboard, bool isGhostMode)
    {
        ...
        if (isGhostMode)
        {
            ...
            // Space = up; Shift = down (Ctrl not used for vertical in ghost mode).
            if (spaceHeld) verticalAxis += 1.0f;
            if (shiftHeld) verticalAxis -= 1.0f;
            // Ctrl does nothing in ghost mode
        }
        else
        {
            // Normal mode: Ctrl = sprint (run), Shift = crouch/sneak
            sprintHeld = ctrlHeld;
            crouchHeld = shiftHeld;
        }
