...
public static class BiomeMapGenerator
{
        ...
        bool mirrorX = false)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"Generating downsampled biome map: {imageSize}x{imageSize} ({cellSize}x{cellSize} blocks/pixel) centered at ({centerX}, {centerZ})...");
        ...
        System.Diagnostics.Trace.TraceInformation($"Biome map saved to: {outputPath}");
        System.Diagnostics.Trace.TraceInformation("Biome distribution:");
        foreach (var (biome, count) in biomeStats.OrderByDescending(kv => kv.Value))
        {
            if (count > 0)
            {
                var percentage = count * 100.0 / totalPixels;
                System.Diagnostics.Trace.TraceInformation($"  {biome}: {count:N0} ({percentage:F1}%)");
            }
        }
    }
        ...
        bool includeClimateOverlay = false)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"Generating full-res biome map: {size}x{size} centered at ({centerX}, {centerZ})...");
        ...
        bool mirrorX = false)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"Generating {parameter} climate map: {imageSize}x{imageSize} ({cellSize}x{cellSize} blocks/pixel)...");
        ...
        image.SaveAsBmp(outputPath);
        System.Diagnostics.Trace.TraceInformation($"Climate map saved to: {outputPath}");
    }
        ...
        bool mirrorX = false)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"Generating height map: {imageSize}x{imageSize} ({cellSize}x{cellSize} blocks/pixel) centered at ({centerX}, {centerZ})...");
        ...
        image.SaveAsBmp(outputPath);
        System.Diagnostics.Trace.TraceInformation($"Height map saved to: {outputPath} (height[min,max]=[{minHeight:F1},{maxHeight:F1}] water={VoxelHelper.WaterLevel} yMax={VoxelHelper.ChunkYSize - 1})");
    }
        ...
        string outputDirectory)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"All debug maps saved to: {outputDirectory}");
    }
