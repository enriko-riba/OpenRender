using DarkVox.Shared.World.Registry; // For Log
...
public sealed class CombatSystem(DroppedItemManager droppedItemManager)
{
        ...
        bool isCreativeMode = false)
    {
        ...
        System.Diagnostics.Trace.TraceInformation($"[Combat] Player hit Mob {mob.Id} ({mob.Definition.Kind}). Dmg: {actualDamage:F1}. HP Left: {mob.Health:F1}/{mob.Definition.MaxHealth}");
        ...
        foreach (var entry in mob.Definition.Drops.Entries)
        {
            if (Random.Shared.NextSingle() <= entry.Probability)
            {
                ...
                if (distSq <= 2.0f * 2.0f)
                {
                    ...
                    System.Diagnostics.Trace.TraceInformation($"[Loot] Auto-collected {count} {entry.Item} for Player");
                }
                else
                {
                    // Drop on ground
                    ...
                    System.Diagnostics.Trace.TraceInformation($"[Loot] Dropped {count} {entry.Item} at {mob.Position}");
                }
            }
        }
    }
