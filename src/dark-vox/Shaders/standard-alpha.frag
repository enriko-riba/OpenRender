...
{ 
    ...
    vec4 diffuseSample = texture(tex.diffuse, texCoord);
    if (diffuseSample.a < 0.05)
    {
        discard;
    }
    ...
    vec3 texDiffuse = diffuseSample.rgb + vertexColor;
    vec3 texDetail = vec3(texture(tex.detail, texCoord * mat.detailTextureScaleFactor));
    ...
    vec3 rgb = mat.emissive.rgb + (Sc + Ac + Dc) * texColor;
    outputColor = vec4(clamp(rgb, 0, 1), diffuseSample.a);
}
